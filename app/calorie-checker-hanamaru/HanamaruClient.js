...
// ※「天ぷら揚げ物」「サイドその他」は中黒なしの安全値(CSVと一致)。タブの表示名だけ中黒入り
const GROUPS = [
  {
    id: "udon",
    labelKey: "group.hanamaruUdon",
    categories: ["うどん"],
  },
  {
    id: "rice",
    labelKey: "group.hanamaruRice",
    categories: ["ご飯物"],
  },
  {
    id: "tempura",
    labelKey: "group.hanamaruTempura",
    categories: ["天ぷら揚げ物"],
  },
  {
    id: "oden",
    labelKey: "group.hanamaruOden",
    categories: ["おでん"],
  },
  {
    id: "topping",
    labelKey: "group.hanamaruTopping",
    categories: ["トッピング"],
  },
  {
    id: "side",
    labelKey: "group.hanamaruSide",
    categories: ["サイドその他"],
  },
];
...
export default function HanamaruClient({ menus, locale = "ja" }) {
  ...
  // selections: { [itemId]: count(数値) | { size: サイズ名 }(サイズ選択商品) }
  //   個数商品=数値, 通常選択=1, サイズ選択商品={size:"中"}
  ...
  // ── 選択方式の判定 ──
  // サイズ選択あり = hasSizeOption=true (うどんで小・中・大などのサイズ選択あり)
  const hasSizeSelect = (item) => item.hasSizeOption === true || item.hasSizeOption === "true";
  // 個数±対象 = トッピング・おでん(複数頼むもの)
  const COUNTABLE_CATEGORIES = ["トッピング", "おでん"];
  const isCountable = (item) =>
    !hasSizeSelect(item) && COUNTABLE_CATEGORIES.some((cat) => fieldMatches(item.category, cat));

  // サイズバリエーションの栄養を取得
  const getSizeVariation = (item, sizeName) => {
    if (!item.sizeVariations) return null;
    try {
      const vars = JSON.parse(item.sizeVariations);
      return vars.find((v) => v.name === sizeName) || null;
    } catch { return null; }
  };
  const getSizeVariations = (item) => {
    if (!item.sizeVariations) return [];
    try { return JSON.parse(item.sizeVariations); } catch { return []; }
  };
  // デフォルトサイズ = 中 > 並盛 > 最初
  const getDefaultSize = (item) => {
    const vars = getSizeVariations(item);
    const std = vars.find((v) => v.name === "中") || vars.find((v) => v.name === "並盛");
    return std ? std.name : (vars[0] ? vars[0].name : null);
  };

  // 1商品の栄養を取得(選択状態を考慮)
  const calcItemNutrition = (item, sel) => {
    // サイズ選択商品: selが{size}ならそのサイズの栄養
    if (hasSizeSelect(item) && sel && typeof sel === "object" && sel.size) {
      const v = getSizeVariation(item, sel.size);
    ...
    return Object.entries(selections)
      ...
      .map(([itemId, sel]) => {
        ...
        return {
          ...
          qty,
          sizeName: (sel && typeof sel === "object" && sel.size) ? sel.size : null,
  ...
  const openSizeModal = (item) => {
    ...
    const curSize = (cur && typeof cur === "object" && cur.size) ? cur.size : getDefaultSize(item);
    setModalState({ itemId: item.id, tempSize: curSize });
  };
  const closeModal = () => setModalState(null);
  const selectSize = (sizeName) => setModalState((prev) => ({ ...prev, tempSize: sizeName }));
  const commitSize = () => {
    if (!modalState) return;
    setSelections((prev) => ({ ...prev, [modalState.itemId]: { size: modalState.tempSize } }));
    setModalState(null);
  };
  ...
  const modalSizes = useMemo(() => {
    if (!modalItem) return [];
    return getSizeVariations(modalItem);
  }, [modalItem]);
  ...
  // チェーン名の表示
  const chainDisplayName = tChain("はなまるうどん") || "はなまるうどん";
  ...
  const groupLabel = (g) => {
    ...
    const fallback = {
      ...
      oden: { ja: "おでん", en: "Oden" },
      topping: { ja: "トッピング", en: "Topping" },
      side: { ja: "サイド・その他", en: "Side & Others" },
    };
      ...
      <div className={styles.wrapper}>
        <header className={styles.header}>
          ...
          <p className={styles.subtitle}>
            {t("chain.subtitleHanamaru") !== "chain.subtitleHanamaru"
              ? t("chain.subtitleHanamaru")
              : "うどん・天ぷら・おでんも。はなまるうどんの全メニューを選ぶだけで、合計カロリーとPFC（たんぱく質・脂質・炭水化物）を瞬時に算出。うどんは小・中・大のサイズを選べ、トッピングやおでんは個数も選べます。アレルゲン情報にも対応しています。"}
          </p>
        </header>
        ...
        <div className={styles.allergyNotice}>
          ...
          <a href="https://www.hanamaruudon.com/" target="_blank" rel="noopener">{locale === "en" ? "Hanamaru Udon " : "はなまるうどん"}{t("chain.officialSite")}</a>
        ...
        <div className={styles.mainLayout}>
          <div className={styles.leftCol}>
            ...
            <div className={styles.menuStack}>
              {visibleMenus.length === 0 ? (
                ...
                visibleMenus.map((item) => {
                  ...
                  const sizeSelect = hasSizeSelect(item);
                  const selectedSize = (sel && typeof sel === "object" && sel.size) ? sel.size : null;
                  ...
                  // 行クリック: サイズ選択商品はダイアログ、通常選択はトグル、個数商品は無効
                  const rowOnClick = sizeSelect
                    ? () => openSizeModal(item)
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={styles.info}>
                        ...
                        {sizeSelect && isSelected && selectedSize && (
                          <span className={styles.sizeBadge}>{t("chain.sizeLabel")} {tOption(selectedSize)}</span>
                        )}
                        {sizeSelect && !isSelected && (
            ...
            <div className={styles.pageFooter}>
              {t("chain.disclaimerPrefix")}<a href="https://www.hanamaruudon.com/" target="_blank" rel="noopener">{locale === "en" ? "Hanamaru Udon " : "はなまるうどん"}{t("chain.officialSite")}</a>{t("chain.disclaimerSuffix")}<br />
              {t("chain.hanamaruDisclaimerAffiliation") !== "chain.hanamaruDisclaimerAffiliation"
                ? t("chain.hanamaruDisclaimerAffiliation")
                : "※当サイトは株式会社花まるおよび「はなまるうどん」とは一切関係のない非公式のカロリー計算ツールです。"}
            </div>
          </div>
        ...
        <div className={styles.modalOverlay} onClick={closeModal}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              <div className={styles.sizeList}>
                {modalSizes.map((r) => (
                  <div
                    key={r.name}
                    className={`${styles.sizeOption} ${modalState.tempSize === r.name ? styles.selected : ''}`}
                    onClick={() => selectSize(r.name)}
                  >
            ...
            <div className={styles.modalFooter}>
              ...
              <button className={styles.modalBtnPrimary} onClick={commitSize}>{t("chain.done")}</button>
            </div>
          </div>
        </div>
      )}

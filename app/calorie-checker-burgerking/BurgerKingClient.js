...
const GROUPS = [
  {
    id: "main",
    labelKey: "group.main",
    categories: ["ワッパー", "ビーフバーガー", "チキンバーガー", "ポークバーガー", "フィッシュバーガー", "ホットドッグ"],
  },
  {
    id: "side",
    labelKey: "group.side",
  ...
  },
  {
    ...
    categories: ["コールドドリンク", "ホットドリンク", "フロートドリンク", "付属品"],
  },
];
...
export default function BurgerKingClient({ menus, locale = "ja" }) {
  ...
  // デフォルトサイズ: レギュラー → M → 8ピース → 最初のサイズ
  const getDefaultSize = (item) => {
    if (!item.sizeVariations) return null;
    try {
      const variations = JSON.parse(item.sizeVariations);
      for (const prefer of ["レギュラー", "M", "8ピース"]) {
  ...
  // チェーン名の表示（英語版は併記）
  const chainDisplayName = tChain("バーガーキング");
        ...
        <div className={styles.mainLayout}>
          <div className={styles.leftCol}>
            ...
            <div className={styles.pageFooter}>
              {t("chain.disclaimerPrefix")}<a href="https://www.burgerking.co.jp/" target="_blank" rel="noopener">{locale === "en" ? "Burger King Japan " : "バーガーキング"}{t("chain.officialSite")}</a>{t("chain.disclaimerSuffix")}<br />
              {t("chain.disclaimerAffiliation")}{locale === "en" ? "Burger King Japan" : "バーガーキング"}{t("chain.disclaimerAffiliationSuffix")}
            </div>
          </div>

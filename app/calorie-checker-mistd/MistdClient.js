...
const GROUPS = [
  {
    id: "main",
    labelKey: "group.mistdMain",
    categories: ["デザート"],
  },
  {
    id: "food",
    labelKey: "group.mistdFood",
    categories: ["サイドメニュー"],
  },
  {
    id: "drink",
    labelKey: "group.mistdDrink",
    categories: ["ドリンク"],
  },
];
...
export default function MistdClient({ menus, locale = "ja" }) {
  ...
  const categoryHref = localizedHref("/category/sweets", locale);

  // チェーン名の表示
  const chainDisplayName = tChain("ミスタードーナツ") || "ミスタードーナツ";
  ...
  const groupLabel = (g) => {
    ...
    const fallback = {
      main: { ja: "ドーナツ・スイーツ", en: "Donuts & Sweets" },
      food: { ja: "パイ・フード", en: "Pies & Food" },
      ...
      <div className={styles.wrapper}>
        <header className={styles.header}>
          <div className={styles.breadcrumb}>
            ...
            <Link href={categoryHref}>{tCategory("スイーツ") || (locale === "en" ? "Sweets / スイーツ" : "スイーツ")}</Link>
            <span className={styles.sep}>/</span>{chainDisplayName}
          </div>
          <h1>{chainDisplayName}</h1>
          <p className={styles.subtitle}>
            {t("chain.subtitleMistd") !== "chain.subtitleMistd"
              ? t("chain.subtitleMistd")
              : "ドーナツ・パイ・ドリンクも。ミスタードーナツの全メニューを選ぶだけで、合計カロリーとPFC（たんぱく質・脂質・炭水化物）を瞬時に算出。個数を変えれば合計も自動計算。アレルゲン情報にも対応しています。"}
          </p>
        </header>
        ...
        <div className={styles.allergyNotice}>
          ...
          <a href="https://www.misterdonut.jp/" target="_blank" rel="noopener">{locale === "en" ? "Mister Donut " : "ミスタードーナツ"}{t("chain.officialSite")}</a>
        ...
        <div className={styles.mainLayout}>
          <div className={styles.leftCol}>
            ...
            <div className={styles.pageFooter}>
              {t("chain.disclaimerPrefix")}<a href="https://www.misterdonut.jp/" target="_blank" rel="noopener">{locale === "en" ? "Mister Donut " : "ミスタードーナツ"}{t("chain.officialSite")}</a>{t("chain.disclaimerSuffix")}<br />
              {t("chain.mistdDisclaimerAffiliation") !== "chain.mistdDisclaimerAffiliation"
                ? t("chain.mistdDisclaimerAffiliation")
                : "※当サイトは株式会社ダスキン（ミスタードーナツ）および「ミスタードーナツ」とは一切関係のない非公式のカロリー計算ツールです。"}
            </div>
          </div>

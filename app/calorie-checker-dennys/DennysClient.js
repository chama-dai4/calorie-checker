...
const GROUPS = [
  ...
  {
    id: "morning",
    labelKey: "group.dennysMorning",
    categories: ["モーニング"],
  },
  {
    id: "lunch",
    labelKey: "group.dennysLunch",
    categories: ["ランチ"],
  },
  {
    id: "dessert",
    labelKey: "group.dennysDessert",
    categories: ["デザート・ドリンク"],
  },
  {
    id: "kids",
    labelKey: "group.dennysKids",
    categories: ["キッズ", "季節・フェア"],
  },
];
...
export default function DennysClient({ menus, locale = "ja" }) {
  ...
  // チェーン名の表示
  const chainDisplayName = tChain("デニーズ") || "デニーズ";
  ...
  const groupLabel = (g) => {
    ...
    const fallback = {
      ...
      kids: { ja: "キッズ・季節", en: "Kids & Seasonal" },
    };
      ...
      <div className={styles.wrapper}>
        <header className={styles.header}>
          ...
          <p className={styles.subtitle}>
            {t("chain.subtitleDennys") !== "chain.subtitleDennys"
              ? t("chain.subtitleDennys")
              : "ハンバーグ・パスタ・モーニングも。デニーズの全メニューを選ぶだけで、合計カロリーとPFC（たんぱく質・脂質・炭水化物）を瞬時に算出。アレルゲン情報も一目で確認できます。"}
          </p>
        </header>
        ...
        <div className={styles.allergyNotice}>
          ...
          <a href="https://www.dennys.jp/" target="_blank" rel="noopener">{locale === "en" ? "Denny's " : "デニーズ"}{t("chain.officialSite")}</a>
        ...
        <div className={styles.mainLayout}>
          <div className={styles.leftCol}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={styles.right}>
                        ...
                        {(item.protein || item.fat || item.carbohydrate) ? (
                          <div className={styles.itemPfc}>
                            {t("chain.protein")} {Math.round((item.protein || 0) * 10) / 10}g · {t("chain.fat")} {Math.round((item.fat || 0) * 10) / 10}g · {t("chain.carbs")} {Math.round((item.carbohydrate || 0) * 10) / 10}g
                          </div>
                        ) : null}
                      </div>
                    </div>
            ...
            <div className={styles.pageFooter}>
              {t("chain.disclaimerPrefix")}<a href="https://www.dennys.jp/" target="_blank" rel="noopener">{locale === "en" ? "Denny's " : "デニーズ"}{t("chain.officialSite")}</a>{t("chain.disclaimerSuffix")}<br />
              {t("chain.dennysDisclaimerAffiliation") !== "chain.dennysDisclaimerAffiliation"
                ? t("chain.dennysDisclaimerAffiliation")
                : "※当サイトは株式会社デニーズジャパンおよびデニーズとは一切関係のない非公式のカロリー計算ツールです。"}
            </div>
          </div>

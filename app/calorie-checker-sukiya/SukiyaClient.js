...
const GROUPS = [
  {
    id: "main",
    labelKey: "group.sukiyaMain",
    categories: ["丼", "カレー"],
  },
  {
    id: "morning",
    labelKey: "group.sukiyaMorning",
    categories: ["朝食"],
  },
  {
    id: "set",
    labelKey: "group.sukiyaSet",
    categories: ["定食", "お得なセット"],
  },
  {
    id: "side",
    labelKey: "group.sukiyaSide",
    categories: ["サイドメニュー", "ご飯・ドレッシング", "お子様セット"],
  },
  {
    id: "drink",
    labelKey: "group.sukiyaDrink",
    categories: ["ドリンク"],
  },
];
...
export default function SukiyaClient({ menus, locale = "ja" }) {
  ...
  // selections: { [itemId]: { sizeName: string|null } }
  ...
  // チェーン名の表示
  const chainDisplayName = tChain("すき家") || "すき家";
  ...
  const groupLabel = (g) => {
    ...
    const fallback = {
      main: { ja: "丼・カレー", en: "Bowls & Curry" },
      morning: { ja: "朝食", en: "Breakfast" },
      set: { ja: "セット・定食", en: "Sets & Meals" },
      side: { ja: "サイド", en: "Sides" },
      drink: { ja: "ドリンク・デザート", en: "Drinks" },
    };
      ...
      <div className={styles.wrapper}>
        <header className={styles.header}>
          <div className={styles.breadcrumb}>
            ...
            <Link href={categoryHref}>{tCategory("牛丼") || (locale === "en" ? "Gyudon / 牛丼" : "牛丼")}</Link>
            <span className={styles.sep}>/</span>{chainDisplayName}
          </div>
          <h1>{chainDisplayName}</h1>
          <p className={styles.subtitle}>
            {t("chain.subtitleSukiya") !== "chain.subtitleSukiya"
              ? t("chain.subtitleSukiya")
              : "牛丼・カレー・定食も。すき家の全メニューを選ぶだけで、合計カロリーとPFC（たんぱく質・脂質・炭水化物）を瞬時に算出。サイズ選択とアレルゲン情報にも対応しています。"}
          </p>
        </header>
        ...
        <div className={styles.allergyNotice}>
          ...
          <a href="https://www.sukiya.jp/" target="_blank" rel="noopener">{locale === "en" ? "Sukiya " : "すき家"}{t("chain.officialSite")}</a>
        ...
        <div className={styles.mainLayout}>
          <div className={styles.leftCol}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={styles.right}>
                        ...
                        {(itemNutri.protein || itemNutri.fat || itemNutri.carb) ? (
                          <div className={styles.itemPfc}>
                            {t("chain.protein")} {Math.round((itemNutri.protein || 0) * 10) / 10}g · {t("chain.fat")} {Math.round((itemNutri.fat || 0) * 10) / 10}g · {t("chain.carbs")} {Math.round((itemNutri.carb || 0) * 10) / 10}g
                          </div>
                        ) : null}
                      </div>
                    </div>
            ...
            <div className={styles.pageFooter}>
              {t("chain.disclaimerPrefix")}<a href="https://www.sukiya.jp/" target="_blank" rel="noopener">{locale === "en" ? "Sukiya " : "すき家"}{t("chain.officialSite")}</a>{t("chain.disclaimerSuffix")}<br />
              {t("chain.sukiyaDisclaimerAffiliation") !== "chain.sukiyaDisclaimerAffiliation"
                ? t("chain.sukiyaDisclaimerAffiliation")
                : "※当サイトは株式会社すき家本部および「すき家」とは一切関係のない非公式のカロリー計算ツールです。"}
            </div>
          </div>

...
const GROUPS = [
  {
    id: "main",
    labelKey: "group.main",
    categories: ["バンズ単品", "ハンバーガー", "期間限定バーガー", "キッズバーガー"],
  },
  {
    id: "side",
    labelKey: "group.side",
    categories: ["サイドメニュー", "フレバル", "フレバル期間限定", "スープ", "デザート"],
  },
  {
    id: "drink",
    labelKey: "group.drink",
    categories: ["ホットドリンク", "コールドドリンク", "アルコール", "季節商品ドリンク"],
  },
];
...
export default function FreshnessClient({ menus, locale = "ja" }) {
  ...
  // デフォルトサイズ: M → R → T → 1倍 → 最初のサイズ
  const getDefaultSize = (item) => {
    if (!item.sizeVariations) return null;
    try {
      const variations = JSON.parse(item.sizeVariations);
      for (const prefer of ["M", "R", "T", "1倍"]) {
        const found = variations.find((v) => v.name === prefer);
  ...
  // チェーン名の表示（英語版は併記）
  const chainDisplayName = tChain("フレッシュネスバーガー");
        ...
        <div className={styles.mainLayout}>
          <div className={styles.leftCol}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={styles.info}>
                        ...
                        {item.isLimited && (
                          <span className={styles.sizeBadge} style={{ marginLeft: 6, backgroundColor: '#f8e8d0', color: '#a0651a' }}>
                            {t("chain.limited") || "店舗限定"}
                          </span>
                        )}
                      </div>
            ...
            <div className={styles.pageFooter}>
              {t("chain.disclaimerPrefix")}<a href="https://www.freshnessburger.co.jp/" target="_blank" rel="noopener">{locale === "en" ? "Freshness Burger Japan " : "フレッシュネスバーガー"}{t("chain.officialSite")}</a>{t("chain.disclaimerSuffix")}<br />
              {t("chain.disclaimerAffiliation")}{locale === "en" ? "Freshness Burger Japan" : "フレッシュネスバーガー"}{t("chain.disclaimerAffiliationSuffix")}
            </div>
          </div>
